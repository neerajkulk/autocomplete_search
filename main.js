...
function filterResults(input, limit = 7) {
    let results = []

    const regexp = RegExp(`^${input}`, 'gi');
...
function resultTemplate(object) {
    return `<a href="#" class="list-group-item list-group-item-action flex-column align-items-start">
        <p class="mb-1">${object.title}</p>
        <small class="text-muted">Completed: ${object.completed}</small>
    </a>`
}
...
function updateDOM(results) {
    ...
    let html = ''
    results.forEach((item) => html += resultTemplate(item))
...
// Placeholder data. Assume we get this from an external API
const data = [
    {
        "userId": 1,
        "id": 1,
